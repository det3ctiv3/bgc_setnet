...
\author{\makebox[\textwidth][c]{\textbf{Rustambek Urokov}$^{1}$ \qquad \textbf{Asifullah Khan}$^{2,3}$ \qquad \textbf{Farhod Eshboyev}$^{1}$}\\[0.3em]
  \makebox[\textwidth][c]{\textbf{Dovud Asadov}$^{1}$ \qquad \textbf{Sejuti Rahman}$^{1}$ \qquad \textbf{Dilafruz Kushokova}$^{1}$}\\[0.5em]
  \makebox[\textwidth][c]{$^{1}$Department of Chemical Engineering \& Materials Sciences, New Uzbekistan University}\\
  \makebox[\textwidth][c]{$^{2}$Biotechnology and Bioinformatics Laboratory, Institute for Advanced Studies}\\
  \makebox[\textwidth][c]{New Uzbekistan University}\\
  \makebox[\textwidth][c]{$^{3}$Department of Biochemistry, Abdul Wali Khan University Mardan, Pakistan}\\[0.3em]
...
Manuscript source, aggregate-analysis and figure-generation code, and compact family-level result artifacts are available at \url{https://github.com/det3ctiv3/bgc_setnet}. The input atlas, frozen ESM-2 embeddings, and exact checkpoints for the reported five-seed experiments are not part of the current public release. The repository records the expected checkpoint and split SHA-256 hashes so that a recovered model archive can be verified.
