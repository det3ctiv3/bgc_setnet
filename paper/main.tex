...
\author{\makebox[\textwidth][c]{\textbf{Rustambek Urokov} \qquad \textbf{Asifullah Khan} \qquad \textbf{Farhod Eshboyev}}\\[0.3em]
  ...
  \makebox[\textwidth][c]{New Uzbekistan University, Tashkent, Uzbekistan}\\[0.3em]
...
We evaluate Pfam-augmented BGC-SetNet with a fixed group-disjoint split and then test two strategies to improve the representation: feeding the model with BGC-level Pfam inventory used by the baseline and learning weights for Pfam domains.
...
\begin{enumerate}
    ...
    \item an empirical analysis of the benchmark, illustrating that explicit Pfam-domain content performs better than the sequence-derived representations. The ESM plus Pfam-augmented BGC-SetNet ensemble shows small, statistically unsupported gains in several secondary metrics.
\end{enumerate}
...
The validation-selected ensemble combines the learned Pfam-augmented BGC-SetNet cosine score and raw ESM cosine score. The selected BGC-SetNet + Pfam weights were $0.1, 0.1, 0.1, 0.2,$ and $0.4$ across the five seeds; the test scores were never used to select these values.
...
Table~\ref{tab:final_results} reports the canonical five-seed aggregate. Pfam Jaccard and weighted Pfam are the strongest methods on the primary Recall@50 metric and are numerically indistinguishable. Pfam-augmented BGC-SetNet is lower than Pfam, while the ESM plus Pfam-augmented BGC-SetNet ensemble is close but lower.
...
BGC-SetNet + Pfam & \shortstack{0.8472\\[-1pt]{\scriptsize[0.776,\,0.912]}} & \shortstack{0.2786\\[-1pt]{\scriptsize[0.180,\,0.391]}} & \shortstack{0.7771\\[-1pt]{\scriptsize[0.661,\,0.884]}} & \shortstack{0.8502\\[-1pt]{\scriptsize[0.772,\,0.923]}} \\
...
\shortstack{ESM + BGC-SetNet\\+ Pfam} & \shortstack{0.8769\\[-1pt]{\scriptsize[0.813,\,0.936]}} & \shortstack{0.3096\\[-1pt]{\scriptsize[0.197,\,0.435]}} & \shortstack{0.8503\\[-1pt]{\scriptsize[0.756,\,0.934]}} & \shortstack{0.9058\\[-1pt]{\scriptsize[0.849,\,0.957]}} \\
...
The ESM plus Pfam-augmented BGC-SetNet ensemble has a Recall@50 difference of $-0.00198$ relative to Pfam Jaccard (exact $p=0.625$; 2 wins, 12 ties, 2 losses). Weighted Pfam Jaccard has a difference of $+0.00003$ (exact $p=0.50$; 2 wins, 14 ties, 0 losses). These are not meaningful improvements.
...
\caption{Per-family Recall@50 compared with Pfam Jaccard. The diagonal denotes parity. The learned ESM plus Pfam-augmented BGC-SetNet ensemble does not systematically exceed Pfam, and weighted Pfam is almost identical to the unweighted score.}
...
We present a reproducible five-seed evaluation framework for sequence-derived BGC retrieval. On 6,953 atlas BGCs and 16 eligible held-out silver groups, Pfam-based methods remain the strongest practical baselines. Neither the Pfam-augmented BGC-SetNet nor its validation-selected ESM plus Pfam-augmented BGC-SetNet ensemble provides a supported improvement, and a differentiable weighted-Pfam generalization is effectively identical to standard Pfam Jaccard. The correct biological interpretation is gene-content diversity relative to references, not proof of alternative biosynthetic pathways. The protocol and aggregate result artifacts provide a useful baseline for future work with curated labels and pathway-level evidence.

\section*{Code and Data Availability}
Manuscript source, the curated bioinformatics data-preparation pipeline, aggregate-analysis and figure-generation code, and compact family-level result artifacts are available at \url{https://github.com/det3ctiv3/bgc_setnet}. Final five-seed Pfam-augmented BGC-SetNet and weighted-Pfam checkpoints, per-seed configs, Pfam vocabularies, evaluation metadata, and a model card are available at \url{https://huggingface.co/whiteh4t/bgc-setnet}. Large raw genome files, generated atlas tables, and the frozen ESM-2 embedding matrix are not bundled in either source repository.
