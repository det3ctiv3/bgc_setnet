...
Manuscript source, the curated bioinformatics data-preparation pipeline, aggregate-analysis and figure-generation code, and compact family-level result artifacts are available at \url{https://github.com/det3ctiv3/bgc_setnet}. Final five-seed Pfam-augmented SetNet and weighted-Pfam checkpoints, per-seed configs, Pfam vocabularies, evaluation metadata, and a model card are available at \url{https://huggingface.co/whiteh4t/bgc-setnet}. Large raw genome files, generated atlas tables, and the frozen ESM-2 embedding matrix are not bundled in either source repository.
